          ...
          children: [
            ...
            ElevatedButton(
                onPressed: () {
                  ...
                  myList = null;
                  myList =
                      getList([dayController, monthController, yearController]);
                },
  ...
  RealmResults<RecordCls>? checkQuery(
    ...
    if (controller[0].text.isNotEmpty) {
      return recordList.realm.query<RecordCls>(
          r" name == $0 and day == $1 SORT(time DESC)",
          [rdoValue, controller[0].text]);
    } else if (controller[1].text.isNotEmpty) {
      return recordList.realm.query<RecordCls>(
          r" name == $0 and month == $1 SORT(time DESC)",
          [rdoValue, controller[1].text]);
    } else if (controller[2].text.isNotEmpty) {
      return recordList.realm.query<RecordCls>(
          r" name == $0 and year == $1 SORT(time DESC)",
          [rdoValue, controller[2].text]);
    } else if (controller[0].text.isNotEmpty && controller[1].text.isNotEmpty) {
      return recordList.realm.query<RecordCls>(
          r" name == $0 and day == $1 and month == $2 SORT(time DESC)",
          [rdoValue, controller[0].text, controller[1].text]);
    } else if (controller[0].text.isNotEmpty && controller[2].text.isNotEmpty) {
      return recordList.realm.query<RecordCls>(
          r" name == $0 and day == $1 and year == $2 SORT(time DESC)",
          [rdoValue, controller[0].text, controller[2].text]);
    } else if (controller[1].text.isNotEmpty && controller[2].text.isNotEmpty) {
      return recordList.realm.query<RecordCls>(
          r" name == $0 and month == $1 and year == $2 SORT(time DESC)",
          [rdoValue, controller[0].text, controller[2].text]);
    } else if (controller[0].text.isNotEmpty &&
        controller[1].text.isNotEmpty &&
        controller[2].text.isNotEmpty) {
      return recordList.realm.query<RecordCls>(
          r" name == $0 and day == $1 and month == $2 and year == $3 SORT(time DESC)",
          [
            rdoValue,
            controller[0].text,
            controller[1].text,
      ...
      return recordList.realm.query<RecordCls>(r" name == $0 SORT(time DESC)", [
        rdoValue,
      ]);
    }
  }
